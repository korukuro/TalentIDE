...
export async function getActiveSessions(req, res) {
    try {
        const session = await Session.findOne({status:"active"}).populate("host","name profileImage email clerkId").sort({createdAt:-1}).limit(20);
...
export async function getMyRecentSessions(req, res) {
    try {
        ...
        const session = await Session.find({status:"completed", $or:[{host:userId},{participants:userId}]}).sort({createdAt:-1}).limit(20);
        res.status(200).json({ session });
    } catch (error) {
